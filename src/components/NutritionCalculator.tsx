...
export default function NutritionCalculator({ isOpen: externalIsOpen, onOpenChange }: NutritionCalculatorProps = {} as NutritionCalculatorProps) {
  ...
  const [history, setHistory] = useState<Array<{ food: string; nutrition: NutritionResult }>>([]);
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      if (response.success && response.nutrition) {
        ...
        setHistory(prev => [{ food: foodInput, nutrition: response.nutrition }, ...prev.slice(0, 4)]);
      ...
      toast.error(error.response?.data?.message || 'Failed to calculate nutrition. Please try again.');
    } finally {
  ...
  const handleHistoryClick = (item: { food: string; nutrition: NutritionResult }) => {
              ...
              {nutritionResult && (
                <div className="space-y-4">
                  <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400">
                    ...
                    <h3 className="font-semibold text-lg">Nutrition Breakdown</h3>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <div className="p-4 rounded-xl bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 border border-blue-200 dark:border-blue-700">
                      ...
                      <p className="text-2xl font-bold text-blue-900 dark:text-blue-100">
                        {nutritionResult.protein.toFixed(1)} <span className="text-sm font-normal">gm</span>
                      </p>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 border border-purple-200 dark:border-purple-700">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-medium text-purple-700 dark:text-purple-300">Carbs</span>
                      </div>
                      <p className="text-2xl font-bold text-purple-900 dark:text-purple-100">
                        {nutritionResult.carbs.toFixed(1)} <span className="text-sm font-normal">gm</span>
                      </p>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-gradient-to-br from-pink-50 to-pink-100 dark:from-pink-900/20 dark:to-pink-800/20 border border-pink-200 dark:border-pink-700">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-medium text-pink-700 dark:text-pink-300">Fats</span>
                      </div>
                      <p className="text-2xl font-bold text-pink-900 dark:text-pink-100">
                        {nutritionResult.fats.toFixed(1)} <span className="text-sm font-normal">gm</span>
                      </p>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/20 dark:to-amber-800/20 border border-amber-200 dark:border-amber-700">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-sm font-medium text-amber-700 dark:text-amber-300">Calories</span>
                      </div>
                      <p className="text-2xl font-bold text-amber-900 dark:text-amber-100">
                        {nutritionResult.calories.toFixed(0)} <span className="text-sm font-normal">kcal</span>
                      </p>
                    </div>
                    ...
                    {nutritionResult.fiber !== undefined && (
                      <div className="p-4 rounded-xl bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-800/20 border border-green-200 dark:border-green-700">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-sm font-medium text-green-700 dark:text-green-300">Fiber</span>
                        </div>
                        <p className="text-2xl font-bold text-green-900 dark:text-green-100">
                          {nutritionResult.fiber.toFixed(1)} <span className="text-sm font-normal">gm</span>
                        </p>
                      </div>
                    )}
                    ...
                    {nutritionResult.sugar !== undefined && (
                      <div className="p-4 rounded-xl bg-gradient-to-br from-red-50 to-red-100 dark:from-red-900/20 dark:to-red-800/20 border border-red-200 dark:border-red-700">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-sm font-medium text-red-700 dark:text-red-300">Sugar</span>
                        </div>
                        <p className="text-2xl font-bold text-red-900 dark:text-red-100">
                          {nutritionResult.sugar.toFixed(1)} <span className="text-sm font-normal">gm</span>
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  {(nutritionResult.sodium || nutritionResult.calcium || nutritionResult.iron || nutritionResult.vitaminC) && (
                    <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                      <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Additional Nutrients</h4>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        ...
                        {nutritionResult.calcium !== undefined && (
                          <div className="text-center p-3 rounded-lg bg-gray-50 dark:bg-gray-700">
                            <p className="text-xs text-gray-600 dark:text-gray-400">Calcium</p>
                            <p className="text-lg font-semibold text-gray-900 dark:text-white">
                        ...
                        {nutritionResult.iron !== undefined && (
                          <div className="text-center p-3 rounded-lg bg-gray-50 dark:bg-gray-700">
                            <p className="text-xs text-gray-600 dark:text-gray-400">Iron</p>
                            <p className="text-lg font-semibold text-gray-900 dark:text-white">
                        ...
                        {nutritionResult.vitaminC !== undefined && (
                          <div className="text-center p-3 rounded-lg bg-gray-50 dark:bg-gray-700">
                            <p className="text-xs text-gray-600 dark:text-gray-400">Vitamin C</p>
                            <p className="text-lg font-semibold text-gray-900 dark:text-white">
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div className="space-y-2">
                    {history.map((item, index) => (
                      ...
                      >
                        <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{item.food}</p>
                        <p className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                          {item.nutrition.calories.toFixed(0)} kcal • {item.nutrition.protein.toFixed(1)}g protein • {item.nutrition.carbs.toFixed(1)}g carbs
                        </p>
                      </button>
                    ))}
                  </div>
